...
#define BC_HASH_OUTPUT_HRBL_STDERR_BUFFER_BYTES ((size_t)256)

static void bc_hash_output_hrbl_emit_stderr(const char* message)
{
    char stderr_buffer[BC_HASH_OUTPUT_HRBL_STDERR_BUFFER_BYTES];
...
static void bc_hash_output_hrbl_digest_hex(bc_hash_algorithm_t algorithm, const bc_hash_result_entry_t* result, char* out_buffer,
                                           size_t* out_length)
{
    switch (algorithm) {
    case BC_HASH_ALGORITHM_CRC32: {
...
static bool bc_hash_output_hrbl_write_file_block(bc_hrbl_writer_t* writer, bc_hash_algorithm_t algorithm, const bc_hash_file_entry_t* entry,
                               ...
                               const bc_hash_result_entry_t* results, const bc_hash_output_context_t* context)
{
    ...
    if (!bc_allocators_context_create(&allocator_config, &memory_context)) {
        bc_hash_output_hrbl_emit_stderr("bc-hash: hrbl output: allocator init failed\n");
        return false;
    }
    ...
    if (!bc_hash_output_hrbl_set_string(writer, "tool", BC_HASH_OUTPUT_HRBL_TOOL_NAME))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_string(writer, "tool_version", tool_version))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_uint64(writer, "schema_version", BC_HASH_OUTPUT_HRBL_SCHEMA_VERSION))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_string(writer, "algorithm", bc_hash_output_hrbl_algorithm_name(algorithm)))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_uint64(writer, "started_at_unix_ms", started_at_unix_ms))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_uint64(writer, "wall_ms", wall_ms))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_uint64(writer, "worker_count", worker_count))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_string(writer, "dispatch_mode", dispatch_mode))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_uint64(writer, "files_total", (uint64_t)entry_count))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_uint64(writer, "files_ok", (uint64_t)files_ok))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_uint64(writer, "files_error", (uint64_t)files_error))
        goto cleanup;
    if (!bc_hash_output_hrbl_set_uint64(writer, "bytes_total", bytes_total))
        goto cleanup;

    if (!bc_hrbl_writer_begin_block(writer, "files", bc_hash_output_hrbl_cstr_length("files")))
        goto cleanup;
    for (size_t index = 0; index < entry_count; ++index) {
        bc_hash_file_entry_t entry;
        if (!bc_containers_vector_get(entries, index, &entry)) {
    ...
    if (!bc_core_writer_write_bytes(output_writer, buffer, buffer_size)) {
        bc_hash_output_hrbl_emit_stderr("bc-hash: hrbl output: writer failed\n");
        goto cleanup;
    }
    success = true;

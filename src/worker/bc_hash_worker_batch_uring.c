...
bool bc_hash_reader_ring_init(bc_hash_reader_ring_t* ring)
{
    ...
    int queue_status = io_uring_queue_init(BC_HASH_RING_QUEUE_DEPTH, &ring->ring, 0);
    if (queue_status < 0) {
        return false;
    }

    int register_status = io_uring_register_files_sparse(&ring->ring, BC_HASH_READER_RING_SLOT_COUNT);
...
static uint64_t bc_hash_ring_encode_user_data(unsigned int slot_index, unsigned int operation_code)
{
    return ((uint64_t)slot_index << 8) | (uint64_t)operation_code;
}

static unsigned int bc_hash_ring_decode_slot(uint64_t user_data)
{
    return (unsigned int)(user_data >> 8);
}

static unsigned int bc_hash_ring_decode_operation(uint64_t user_data)
{
                                     ...
                                     void* buffer_address, unsigned char* probe_byte_address)
{
    ...
    io_uring_sqe_set_data64(open_sqe, bc_hash_ring_encode_user_data(slot_index, BC_HASH_RING_OP_OPENAT));
...
static bool bc_hash_ring_fits_in_slot(const bc_hash_reader_batch_item_t* item)
{
    return item->file_size > 0 && item->file_size <= BC_HASH_READER_RING_SLOT_BUFFER_BYTES;
}
                                    ...
                                    size_t* slot_bytes_read, bc_hash_reader_consumer_fn_t consumer_function, struct io_uring_cqe* cqe)
{
    ...
    unsigned int operation_code = bc_hash_ring_decode_operation(user_data);
    int cqe_result = cqe->res;
    size_t item_index = slot_to_item[slot_index];
    bc_hash_reader_batch_item_t* item = &items[item_index];

    if (operation_code == BC_HASH_RING_OP_OPENAT) {
        if (cqe_result < 0) {
            item->success = false;
    ...
    if (operation_code == BC_HASH_RING_OP_READ) {
        if (cqe_result < 0) {
...
static bool bc_hash_ring_drive_chunk(bc_hash_reader_ring_t* ring, bc_hash_reader_batch_item_t* items, size_t chunk_start,
                                     size_t chunk_count, bc_hash_reader_consumer_fn_t consumer_function)
{
    ...
    for (size_t offset = 0; offset < chunk_count; ++offset) {
        ...
        const bc_hash_reader_batch_item_t* item = &items[item_index];
        if (!bc_hash_ring_fits_in_slot(item)) {
            continue;
        }
...
static void bc_hash_ring_fallback_sync(bc_hash_reader_batch_item_t* item, bc_hash_reader_consumer_fn_t consumer_function)
{
    int fallback_errno = 0;
    if (bc_hash_reader_consume_file(item->absolute_path, item->file_size, item->consumer_context, consumer_function, &fallback_errno)) {
        item->success = true;
        item->errno_value = 0;
    } else {
        item->success = false;
                                  ...
                                  bc_hash_reader_consumer_fn_t consumer_function)
{
    ...
    while (processed < item_count) {
        ...
        size_t chunk_count = chunk_remaining < BC_HASH_READER_RING_SLOT_COUNT ? chunk_remaining : BC_HASH_READER_RING_SLOT_COUNT;
        if (!bc_hash_ring_drive_chunk(ring, items, processed, chunk_count, consumer_function)) {
            for (size_t offset = 0; offset < chunk_count; ++offset) {
                bc_hash_reader_batch_item_t* item = &items[processed + offset];
                if (!item->success && item->errno_value == 0) {
                    bc_hash_ring_fallback_sync(item, consumer_function);
                }
            }
        ...
        for (size_t offset = 0; offset < chunk_count; ++offset) {
            ...
            if (bc_hash_ring_fits_in_slot(item) && item->errno_value != 0) {
                continue;
            }
            bc_hash_ring_fallback_sync(item, consumer_function);
        }
